...
3. If criteria pass, it removes both `bodyA` and `bodyB` via `Composite.remove`, looks up the next tier data in the respective Theme array, calculates the exact mathematical center point, and instantly spawns a new upgraded body via `Composite.add`.
